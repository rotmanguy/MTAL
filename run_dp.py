...
def main():
    ...
    parser.add_argument("--tgt_domains", type=str,  nargs='+', help="The target data names.", default=None)
    ...
    parser.add_argument("--scale_temperature", default=False, action="store_true",
                        help="Whether to perform post-processing of temperature scaling.")
    ...
    # else:
    #     num_iters = 1
    # Todo check if sample_ids are generated always the same

    if args.do_eval:
        iterations = range(args.num_al_iterations - 1, args.num_al_iterations)
    else:
        iterations = range(args.num_al_iterations)
    ...
    for al_iter_num in iterations:
        ...
        sample_ids = utils.training.train(args, dataloaders, vocab, al_iter_num)
