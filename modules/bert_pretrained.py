...
class WordpieceIndexer(TokenIndexer[int]):
    ...
    def __init__(self,
                 ...
                 wordpiece_tokenizer: Callable[[str], List[str]] = None,
        ...
        if self.tokenizer is not None:
            ...
            self._start_piece_ids = [self.tokenizer._convert_token_to_id(wordpiece)
                                     ...
                                     for wordpiece in self.wordpiece_tokenizer(token)]
            ...
            self._separator_ids = [self.tokenizer._convert_token_to_id(wordpiece)
                                   for wordpiece in self.wordpiece_tokenizer(separator_token)]
        else:
            # Convert the start_tokens and end_tokens to wordpiece_ids
...
@TokenIndexer.register("bert-pretrained-indexer")
class PretrainedBertIndexer(WordpieceIndexer):
    ...
    def __init__(self,
        ...
        if 'roberta' in pretrained_model.lower():
            bert_tokenizer = RobertaTokenizer.from_pretrained(pretrained_model, do_lower_case=do_lowercase)
            tokenizer = bert_tokenizer
            wordpiece_tokenizer = bert_tokenizer._tokenize
            bert_vocab = bert_tokenizer.encoder
            never_lowercase = []
            start_tokens = ["<s>"]
            end_tokens = ["</s>"]
            separator_token = "</s>"
            ...
            never_lowercase = _NEVER_LOWERCASE
            start_tokens = ["[CLS]"]
            end_tokens = ["[SEP]"]
            separator_token = "[SEP]"
...
class BertEmbedder(TokenEmbedder):
    ...
    combine_layers : str, optional (default: "all")
        Options: "last", "all"
    """
    def __init__(self,
                 bert_model,
    ...
    def forward(self,
        ...
        try:
            _, _, all_encoder_layers = self.bert_model(input_ids=util.combine_initial_dims(input_ids),
                                                    token_type_ids=util.combine_initial_dims(token_type_ids),
                                                    attention_mask=util.combine_initial_dims(input_mask),
                                                    )
        except:
            _, all_encoder_layers = self.bert_model(input_ids=util.combine_initial_dims(input_ids),
                                                    attention_mask=util.combine_initial_dims(input_mask),
                                                    )

        if isinstance(all_encoder_layers, list) or isinstance(all_encoder_layers, tuple):
...
@TokenEmbedder.register("bert-pretrained-embedder")
class PretrainedBertEmbedder(BertEmbedder):
    def __init__(self, pretrained_model: str,
        ...
        # hack to enforce output_hidden_states to be true
        if 'roberta' in pretrained_model.lower():
            model = RobertaModel.from_pretrained(pretrained_model, config=self.config)
        else:
            model = BertModel.from_pretrained(pretrained_model, config=self.config)
    ...
    def set_dropout(self, dropout):
        ...
        self.bert_model.embeddings.dropout.p = dropout
