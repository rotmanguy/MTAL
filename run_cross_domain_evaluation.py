...
def call_func(src_domain,
    ...
    if tgt_domains is not None:
        shell_str += ['--tgt_domains', ' '.join(tgt_domains)]
    ...
    if not os.path.exists(os.path.join(model_name, src_domain + '_res_test_iter_' + str(num_al_iterations - 1) + '.conllu')) or do_mode == 'eval':
        ...
        for domain in domains:
            for tasks in tasks_list:
                ...
                if len(tasks) > 1:
                    ...
                    al_scoring_list = ['joint_max_entropy', 'joint_min_entropy']
                else:
                    continue
                ...
                for unfreeze_mode in unfreeze_list:
                    for al_scoring in al_scoring_list:
                        ...
                        tgt_domains_ = [tgt_domain for tgt_domain in tgt_domains if tgt_domain != domain]
                        for task_for_scoring in task_for_scoring_list:
                            command = call_func(dataset=dataset, vocab_path=vocab_path, bert_vocab_path=bert_vocab_path,
                                                src_domain=domain, tgt_domains=tgt_domains_, tasks=tasks, task_levels=task_levels, unfreeze_mode=unfreeze_mode,
