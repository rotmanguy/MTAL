...
if use_gpu:
    ...
    ray.init(num_gpus=torch.cuda.device_count(), temp_dir='tmp')
else:
...
def call_func(src_domain,
    ...
    shell_str = ['python run_dp.py',
                 ...
                 ' --max_seq_length 128 --per_gpu_train_batch_size 4 --gradient_accumulation_steps 8 --seed 1 --learning_rate 1e-4 --weight_decay 0.01  --dropout 0.33 --bert_dropout 0.15 --layer_dropout 0.1 --word_dropout 0.2 --bert_combine_layers all --mix_embedding',
    ...
    if dataset == 'ud':
        ...
        shell_str += ['--max_steps 15000 --warmup_steps 500 --eval_steps 500']
    ...
    if len(task_levels) > 1:
        if task_levels[1:] == task_levels[:-1]:
            model_name += '_multitask'
        else:
            model_name += '_hierarchical'
    ...
    if pretrained_tasks is not None:
        model_name += '_pretrained_tasks_' + '_'.join(pretrained_tasks)
        if load_model_path_bool:
            model_name += '_loaded'
    ...
    for domain in domains:
        for tasks in tasks_list:
            for unfreeze_mode in unfreeze_list:
                ...
                pretrained_tasks = None
                calls_list.append(call_func.remote(
                    dataset=dataset, vocab_path=vocab_path, bert_vocab_path=bert_vocab_path,
                    src_domain=domain, tgt_domain=domain, tasks=tasks, task_levels=task_levels, unfreeze_mode=unfreeze_mode, load_pretrained_model=load_model_path,
                    do_mode=do_mode, pretrained_model=pretrained_model, pretrained_tasks=pretrained_tasks, training_set_percentage=training_set_percentage))
ray.get(calls_list)