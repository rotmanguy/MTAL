...
printf OUT "  ----------------+------+---------+--------+------------+---------------+-------+\n";
printf OUT "  deprel          | gold | correct | system | recall (%%) | precision (%%) | F1 (%%)  \n";
printf OUT "  ----------------+------+---------+--------+------------+---------------+-------+\n";
foreach my $dep (sort keys %{$counts{all_dep}}) {
    ...
    if (0 < ($rec + $prec)) {
    $f1 = sprintf("%.2f", 2 * $rec * $prec / ($rec + $prec));
    }
    printf OUT "  %-15s | %4d | %7d | %6d | %10s | %13s | %4s\n",
    $dep, $tot_g, $tot_corr, $tot_s, $rec, $prec, $f1;
}
...
printf OUT "  ----------------+------+---------+--------+------------+---------------+-------+\n";
printf OUT "  direction       | gold | correct | system | recall (%%) | precision (%%) | F1 (%%) \n";
printf OUT "  ----------------+------+---------+--------+------------+---------------+-------+\n";
foreach my $dir ('to_root', 'left', 'right', 'self') {
    ...
    printf OUT "  %-15s | %4d | %7d | %6d | %10s | %13s | %4s\n",
    $dir, $tot_g, $tot_corr, $tot_s, $rec, $prec, $f1;
}

printf OUT "\n\n";
printf OUT "  Precision and recall of binned HEAD distance\n\n";
printf OUT "  ----------------+------+---------+--------+------------+---------------+-------+\n";
printf OUT "  distance        | gold | correct | system | recall (%%) | precision (%%) | F1 (%%) \n";
printf OUT "  ----------------+------+---------+--------+------------+---------------+-------+\n";
foreach my $dist ('to_root', '1', '2', '3-6', '7-...') {
    ...
    printf OUT "  %-15s | %4d | %7d | %6d | %10s | %13s | %4s\n",
    $dist, $tot_g, $tot_corr, $tot_s, $rec, $prec, $f1;
}
