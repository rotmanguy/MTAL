...
def evaluate(gold_ud, system_ud):
    ...
    return {
        "Tokens": vars(spans_score(gold_ud.tokens, system_ud.tokens)),
        "Sentences": vars(spans_score(gold_ud.sentences, system_ud.sentences)),
        "Num_Tokens": len(system_ud.tokens),
        "Num_Sentences": len(system_ud.sentences),
        "Words": vars(alignment_score(alignment)),
        "UPOS": vars(alignment_score(alignment, lambda w, _: w.columns[UPOS])),
        "XPOS": vars(alignment_score(alignment, lambda w, _: w.columns[XPOS])),
        "UFeats": vars(alignment_score(alignment, lambda w, _: w.columns[FEATS])),
        "AllTags": vars(alignment_score(alignment, lambda w, _: (w.columns[UPOS], w.columns[XPOS], w.columns[FEATS]))),
        "Lemmas": vars(alignment_score(alignment, lambda w, ga: w.columns[LEMMA] if ga(w).columns[LEMMA] != "_" else "_")),
        "UAS": vars(alignment_score(alignment, lambda w, ga: ga(w.parent))),
        "LAS": vars(alignment_score(alignment, lambda w, ga: (ga(w.parent), w.columns[DEPREL]))),
        "CLAS": vars(alignment_score(alignment, lambda w, ga: (ga(w.parent), w.columns[DEPREL]),
                                filter_fn=lambda w: w.is_content_deprel)),
        "MLAS": vars(alignment_score(alignment, lambda w, ga: (ga(w.parent), w.columns[DEPREL], w.columns[UPOS], w.columns[FEATS],
                                                         [(ga(c), c.columns[DEPREL], c.columns[UPOS], c.columns[FEATS])
                                                          for c in w.functional_children]),
                                filter_fn=lambda w: w.is_content_deprel)),
        "BLEX": vars(alignment_score(alignment, lambda w, ga: (ga(w.parent), w.columns[DEPREL],
...
def write_results_file(evaluation, output_file):
        ...
        for metric in ["Num_Tokens", "Num_Sentences", "Tokens", "Sentences", "Words", "UFeats", "AllTags", "Lemmas", "CLAS", "MLAS",
            ...
            if metric in evaluation:
                if metric not in ["Num_Tokens", "Num_Sentences"]:
                    f.write('\n')
                    f.write("{:11} |{:10.2f} |{:10.2f} |{:10.2f} |{}".format(
                        metric if metric not in special_keys else special_keys[metric],
...
def evaluate_wrapper(args):
    ...
    if args.write_evaluation_file:
        write_results_file(evaluation, args.output_file)
    return evaluation
...
def main():
    ...
    parser.add_argument("--write_evaluation_file", "-w", default=False, action="store_true",
                        help="Write evaluation to a text file")
